...
PasswordStrings = \
    '*,wrong password,The specified password is incorrect,encrypted headers'
...
def start_check():
    ...
    if not ('NZBNA_EVENT' in os.environ or 'NZBPP_DIRECTORY'
            in os.environ) or not 'NZBOP_ARTICLECACHE' in os.environ:
    ...
    if 'NZBPP_DIRECTORY' in os.environ \
        and not os.path.exists(os.environ.get('NZBPP_DIRECTORY')):
    ...
    if not optname in os.environ:
        print '[ERROR] Option %s is missing in configuration file. Please check script settings' \
            % optname[6:]
...
def connect_to_nzbget():
    ...
    xmlRpcUrl = 'http://%s:%s@%s:%s/xmlrpc' % (username, password,
            host, port)
...
def main():
    ...
    tmp_file_name = os.environ.get('NZBOP_TEMPDIR') \
        + '/PasswordDetector/' + os.environ.get(Prefix + 'NZBID')
    ...
    if os.environ.get('NZBNA_EVENT') == 'NZB_ADDED' \
        ...
        if not os.environ.get('NZBPR_FAKEDETECTOR_SORTED') == 'yes':
            print '[INFO] Sorting inner files for earlier fake detection for %s' \
                % NzbName
            sys.stdout.flush()
            sort_inner_files()
